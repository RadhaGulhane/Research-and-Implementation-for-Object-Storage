...
map<int, node*> flush(node* root)
{
	map<int, node*> result;
	/**
	 * if root buffer full and it is leaf node, split it as b tree and 
	 * distribute the messages in buffer 
	 */
	...
	{
		/**
		 * find one child and flush. For more optimization, timestamp can be 
		 * added to find situable child
		 */
		map<int, node*> ::iterator itr;
		...
		{  /* if buff exceed size result = flush(child) */
			...
			/** 
			 * if pivot aaray size exceeds (overflow condition), split that 
			 * node else return result 
			 */
...
/**
 * @brief Serches for value for given key
 * 
 * @param root root node of B-epsilon tree
 * @param key key to search for value associated with it
 * @return int value if key exists else return -1
 */
int searchKey(node* root, int key) 
{
	...
	else
	{
		...
		{
			/* look for appropriate child pointer to search the key */
			map<int, node*> ::iterator itr;
